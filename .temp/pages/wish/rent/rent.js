...
export default class Rent extends Taro.Component {
  ...
  state = {
    preferences: [{ value: 1, text: '燃油车', checked: false, more: '' }, { value: 2, text: '新能源车', checked: false, more: '' }, { value: 3, text: '短租期', checked: false, more: '(3个月)' }, { value: 4, text: '租车跑快车', checked: false, more: '(注重车辆性价比)' }, { value: 5, text: '租车跑专车', checked: false, more: '(注重车辆品牌性能)' }],
  ...
  componentDidMount() {
    ...
    window.onresize = function () {
      if (document.body.scrollHeight < h) {
        ...
        // }, 500);
      } else document.getElementsByClassName("rent-submit")[0].style.display = "block";
    };
  }
  ...
  render() {
    ...
    return <View className="container-rent">
        ...
        <View className="rent-name-view">
          ...
          <AtInput className="rent-name-input" type="text" placeholder="请输入真实姓名" border={false} maxLength={10} onFocus={this.onInputFocus.bind(this)} onBlur={this.onInputBlur.bind(this)} value={name} onChange={this.onNameChange.bind(this)} />
        ...
        <View className="rent-name-view">
          ...
          <AtInput className="rent-name-input" type="phone" placeholder="请输入联系方式" border={false} maxLength={11} onFocus={this.onInputFocus.bind(this)} onBlur={this.onInputBlur.bind(this)} value={phoneNumber} onChange={this.onPhoneNumberChange.bind(this)} />
  ...
  onSubmit(value) {
    ...
    if (valueArray.length <= 0) {
      _showToast({ title: '请勾选出行方案内容', icon: 'none', duration: 2000 });
      return;
    }
    if (name.length < 1) {
      _showToast({ title: '请输入您的称呼方式', icon: 'none', duration: 2000 });
      return;
    }
