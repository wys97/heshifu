...
export default class Points extends Taro.Component {
  ...
  getCityPoints(cityCode, latitude, longitude) {
    ...
    Netservice.request({
      ...
      success: function (res) {
        ...
        if (pointList.length <= 0) _showToast({ title: '暂无符合条件的取餐点', icon: 'none', duration: 2000 });
  ...
  render() {
        ...
        <View className="Points-conent_wrap">
          <View className="Points-conent_left">
            ...
            <View className="Points-distance">
              {value.number >= 1000 ? <Text><Text className="Points-distance_number">{(value.number / 1000).toFixed(1)}</Text>km</Text> : <Text><Text className="Points-distance_number">{value.number}</Text>m</Text>}
            </View>
          ...
          <View className="Points-conent_right">
            ...
            {!value.state && <View className="Points-breathingSpace-btn">
    ...
    return <View className="default_points">
