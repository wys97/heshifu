...
export default class HomePoints extends Taro.Component {
  ...
  render() {
    ...
    const points = pointList.map((value, index) => {
      return <View className="pl-item" key={value.id} onClick={this.toOrder.bind(this, value)}>
        <View className="plih-img_wrap">
          {value.state ? <Image src={value.storeUrl !== null && value.storeUrl !== '' ? value.storeUrl : default_store_img} className="plih-img" /> : <Image src={resting} className="plih-img" />}
        </View>
        <View className="pli_item_content">
          ...
          <View className="pli-address_wrap">
            ...
            <View className="pliht-distance">
              {value.number >= 1000 ? <Text>
                  {(value.number / 1000).toFixed(1)}<Text className="points_distance_unit"> km</Text>
                </Text> : <Text>{value.number}<Text className="points_distance_unit"> m</Text></Text>}
            </View>
          </View>
    ...
    return <View className="container-points">
        {/*固定头部*/}
        ...
        {!noShop ? <View className="points-list">
          ...
          {points}
        </View> : <Image src={noServiceImg} className="points_noServiceImg" style="pointer-events: none" />}
      </View>;
  }
