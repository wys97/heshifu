...
//优惠券组件
export default class TooltipCoupon extends Component {
    ...
    componentWillMount() {
        ...
        if (!userId || couponsData == '' || couponsData.length <= 0) {
            ...
            //如果数据大于2， 只选前两个
            if (okCoupons.length > 2)
                showCoupon = okCoupons.slice(0, 2);
    ...
    render() {
        ...
        return (
            <View>
                {couponState &&
                    <View className={notScroll == 1 ? 'TooltipCoupon2' : 'TooltipCoupon'}>
                        <View className={notScroll == 1 ? 'TooltipCoupon_bg2' : 'TooltipCoupon_bg'}>
                            <View className='CouponImg_wrap'>
                                <View className='CouponImg_wrap_content' onClick={this.closeTooltip.bind(this)}>
                                    {faceValueThanZero.length >= 2 ?
                                        <Image src={CouponImg} className='CouponImg' style='pointer-events: none' /> :
                                        <Image src={commonCouponImg} className='CouponImg' style='pointer-events: none' />
                                    }
                                    ...
                                    {couponsTitle.map((item, index) => {
                                        return <View className='coupon_title' key={index}>
                                            {faceValueThanZero.length >= 2 ?
                                                <Text>恭喜获得大礼包</Text> :
                                                <Text>{item.id === receiveType[0] ? <Text>{item.title}</Text> : ''}</Text>
                                            }
                                        </View>
                                    })
                                    }
    ...
    closeTooltip(e) {
        ...
        if (this.state.afterClose == 1)
            Taro.navigateBack();


        if (this.props.onCloseCouponView)
            this.props.onCloseCouponView();
