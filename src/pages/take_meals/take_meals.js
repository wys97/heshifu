...
export default class TakeMeals extends Component {
  ...
  getMeals() {
    ...
    Netservice.request({
      url: 'heque-eat/eat/no_meal_order_info',
      method: 'GET',
      data:{
        userId
      },
  ...
  // 返回首页点餐
  goIndex() {
    Taro.switchTab({ url: '/pages/index/index' + '?v=' + new Date().getTime() })
  }
  ...
  goHistoryOrder() {
    let userId = localStorage.getItem('userId');
    if (userId)
      Taro.navigateTo({ url: '/pages/my/order_history/order_history' + '?v=' + new Date().getTime() })
    else
  ...
  contactPoint(e) {
    ...
    let { meals, orderIndex } = this.state;
    let order = meals[orderIndex];
    Taro.makePhoneCall({ phoneNumber: order.storePhoneNumber + '' })
  }
  ...
  getWeChatJSApiParam() {
    Netservice.request({
      url: 'heque-eat/we_chat_public_number/get_signature',
      method: 'GET',
      data:{
    ...
    Netservice.request({
      ...
      success: function (res) {
        ...
        if (parseInt(parseFloat(totalPrice) * 100) > parseInt(0))
          that.goWechatPay(totalPrice);
        else
          that.zeroPay(totalPrice);
    ...
    Netservice.request({
      ...
      success: res => {
        if (res.code == Common.NetCode_NoError) {
          ...
          if (zeroCouponIds.length > 0)
            that.readCoupons(zeroCouponIds);
  ...
  cancleOrder() {
    ...
    Netservice.request({
      ...
      fail: function (error) {
        Taro.hideLoading();
        Taro.showToast({ title: '取消订单失败', icon: 'none', duration: 2000 });
      }
    })
  }
