...
import { payment, zeroPayment } from '../../../utils/payment.js'
...
export default class PointBuy extends Component {
    ...
    render() {
        ...
        const affirmPaymentState = <View className={!alterStyle ? 'pt2-order2' : 'pt2-order2_true'}>
            ...
            <View className='pt2o-line2'>
                ...
                <AtInput className='pt2ol-amount2' type='digit' placeholder='请输入金额' border={false} maxLength={6}
                    onFocus={this.onAmountFocus.bind(this)} onBlur={this.onAmountBlur.bind(this)}
                    value={amount} onChange={this.onAmountChange.bind(this)} />
            </View>
            ...
            {!paymentBtnStyle ?
                <View className='po2ol_payment_btn'>确认付款</View> :
                <View onClick={this.bookingMeal.bind(this)} className='po2ol_payment_btn2'>
        ...
        return (
            <View className='Dierct'>
                    ...
                    <View className='pt2pt_head'>
                        ...
                        <View className='pt2pt_head_distance'>距您
                            {storesDetails.number >= 1000 ?
                                <Text className='pt2pt_head_number'>
                                    {(storesDetails.number / 1000).toFixed(1)}<Text className='pt2pt_distance_unit'> km</Text>
                                </Text> :
                                <Text className='pt2pt_head_number'>{storesDetails.number}<Text className='pt2pt_distance_unit'> m</Text></Text>
                            }
                        ...
                        {storesDetails.suppleTime1 !== undefined ?
                            <View className='pt2pt_head_time'>
                                <Text>营业时间：</Text>
                ...
                {!inRest ?
                    ...
                    <View className='inRest_img_wrap2' onClick={this.goToPoints.bind(this)} >
    ...
    formatAmount1(value) {
        ...
        else if (xiaoLength == 1)
            return (Math.round(value * Math.pow(10, 1)) / Math.pow(10, 1)).toFixed(2);
        else
        ...
        if (userId) {
            ...
            Netservice.request({
                ...
                success: res => {
                    ...
                    Taro.showToast({ title: '下单成功', icon: 'success', duration: 2000 });
    ...
    checkPay(orderId) {
        ...
        Netservice.request({
            ...
            success: function (res) {
                ...
                if (parseInt(parseFloat(totalPrice) * 100) > parseInt(0))
                    that.goWechatPay(totalPrice, orderId);
                else
                    that.zeroPay(totalPrice, orderId);

            },
            error: function (err) {
                Taro.hideLoading();
                Taro.showToast({ title: '查询支付金额失败', icon: 'none', duration: 2000 });
            }
        })
    }
    //微信支付
    goWechatPay(totalPrice, orderId) {
        ...
        payment(userCardMedalId, orderId, totalPrice, openId,
            ...
            () => {
                ...
                setGlobalData('currentCoupon', {});
                this.setState({ currentCoupon: {}, amount: '', paymentState: false, alterStyle: false });
                Taro.showToast({ title: '支付取消', icon: 'none', duration: 2000 });
                Taro.navigateTo({ url: '/pages/my/order_history/the_order_details/the_order_details?orderId=' + orderId })
