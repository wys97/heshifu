    ...
    return (<View className='container-book'>
      ...
      <View className='book-point'>
        ...
        <View className='bp-info'>
          ...
          {currentPoint.number >= 1000 ?
              <Text className='bpi-address-distance'>
                {(currentPoint.number / 1000).toFixed(1)} <Text className='booking_unit'>km</Text>
              </Text> :
              <Text className='bpi-address-distance'>{currentPoint.number}<Text className='booking_unit'>m</Text>
              </Text>
            }
            <Text className='margin_10'>|</Text>  {currentPoint.adds}
          </View>
          {businessHours1 !== undefined ?
            <View className='bpi_businessHours'>
    ...
    Netservice.request({
      ...
      success: function (res) {
        ...
        if (parseInt(parseFloat(totalPrice) * 100) > parseInt(0))
          that.goWechatPay(totalPrice, orderId);  //调起微信支付
        else
          that.zeroPay(totalPrice, orderId);  // ；零元支付
      },
      error: function (err) {
        Taro.hideLoading();
