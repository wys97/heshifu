...
import TooltipCoupon from '../../tooltipCoupon/tooltipCoupon.js'

export default class ChoiceMotormanType extends Component {
    ...
    render() {
        ...
        return (
            <View className='ChoiceMotormanType'>
                <View className='Motorman_title'>请选择司机类型</View>
                <View className='MotormanType_wrap' onClick={() => { this.ChangeState(1) }}>
                    {!defaultState ?
                        <Image src={whetherDefaultState === 1 ? a_slct : a_unslct} className='MotormanType_defaultBgImg' /> :
                        <Image src={a_default} className='MotormanType_defaultBgImg' />
                    }
                </View>
                <View className='MotormanType_wrap' onClick={() => { this.ChangeState(2) }}>
                    {!defaultState ?
                        <Image src={whetherDefaultState === 2 ? b_slct : b_unslct} className='MotormanType_defaultBgImg' /> :
                        <Image src={b_default} className='MotormanType_defaultBgImg' />
                    }
                </View>
                <View className='MotormanType_wrap' onClick={() => { this.ChangeState(3) }}>
                    {!defaultState ?
                        <Image src={whetherDefaultState === 3 ? c_slct : c_unslct} className='MotormanType_defaultBgImg' /> :
                        <Image src={c_default} className='MotormanType_defaultBgImg' />
                    }
                </View>
                <View className='Motorman_btn' onClick={this.completeChoice.bind(this)}>
                    {
                        !whetherDefaultState == 0 ?
                            <Image src={btn_next} className='Motorman_btn_img' /> :
                            <Image src={btn_unavailable} className='Motorman_btn_img' />
                    }
                </View>
