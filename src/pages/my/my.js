...
export default class My extends Component {
  ...
  render() {
    ...
    return (
      <View className='My'>
        {userId === null || userId === '' ?
          <View className='myMessage' onClick={this.goLogin}>
  ...
  toApplyList(e) {
    ...
    if (userId)
      Taro.navigateTo({ url: '/pages/my/apply/apply' + '?v=' + new Date().getTime() })
    else
  ...
  logOut() {
    ...
    Netservice.request({
      ...
      success: res => {
        ...
        Taro.removeStorageSync('dishesList');
        Taro.removeStorageSync('orderId');
        Taro.removeStorageSync('listDataState');
        Taro.removeStorageSync('orderState');
  ...
  getWeChatJSApiParam() {
    Netservice.request({
      ...
      success: res => {
        ...
        JSWX.ready(function () {   //需在用户可能点击分享按钮前就先调用
