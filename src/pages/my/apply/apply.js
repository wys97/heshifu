...
export default class Apply extends Component {
  ...
  getMessages() {
    ...
    let that = this;
    Netservice.request({
      url: 'heque-backend/collect/queryIssueList',
      method: 'GET',
      data:{
        userId,
        pageSize:10,
  ...
  render() {
    ...
    return (
      <View className='container-apply'>
  ...
  cancelItem(item) {
    ...
    let url = '';
    if (item.type == 0)
      url = 'heque-backend/collect/changeLoanInfo';
    else if (item.type == 1)
      url = 'heque-backend/work_driver/updateWorkDriverState';
    else if (item.type == 2)
      url = 'heque-backend/employ_driver/updateEmployDriverState';
    else if (item.type == 3)
      url = 'heque-backend/collectCarInfo/handle';
    else if (item.type == 4)
      url = 'heque-backend/collectYaDiInfo/handle';
    else if (item.type == 5)
      url = 'heque-backend/collectCaoCaoInfo/handle';
    else if (item.type == 6)
      url = 'heque-backend/collectShouQiInfo/handle';
    else if (item.type == 7)
      url = 'heque-backend/collectPreciseCarInfo/handle';
