...
export default class CaocaoApply extends Component {
    ...
    componentDidMount() {
        ...
        window.onresize = function () {
            if (document.body.scrollHeight < h)
                document.getElementsByClassName("ApplyDetails_btn_submit_wrap")[0].style.display = "none";
            else
                document.getElementsByClassName("ApplyDetails_btn_submit_wrap")[0].style.display = "block";
        };
    }
        ...
        return (
            <View className='applyDetails'>
                ...
                <View className='ApplyDetails_content_wrap'>
                    ...
                    <Input className='ApplyDetails_input'  type='text' placeholder='请输入真实姓名' maxLength='7'
                        onChange={this.setNameValue.bind(this)} onBlur={this.onInputBlur.bind(this)} />
                    ...
                    <Input className='ApplyDetails_input' value={IdCodeValue} type='number' maxLength='18' placeholder='请输入您的身份证号'
                        onChange={this.setIdCodeValue.bind(this)} onBlur={this.onInputBlur.bind(this)} />
                    ...
                    <Input className='ApplyDetails_input' value={phoneNumber} type='number' maxLength='11' placeholder='请输入手机号码'
                        onChange={this.setPhoneNumber.bind(this)} onBlur={this.onInputBlur.bind(this)} />
                </View>
    ...
    submitBtn(e) {
        ...
        if (name.length < 1) {
            Taro.showToast({ title: '请输入你的姓名', icon: 'none', duration: 1500 });
            return;
        }
        if (genderValue < 0) {
            Taro.showToast({ title: '请选择性别', icon: 'none', duration: 1500 });
            return;
        }
        if (IdCodeValue.length < 18) {
            Taro.showToast({ title: '请输入正确的身份证号', icon: 'none', duration: 1500, });
            return;
        }
        if (ageValue < 0) {
            Taro.showToast({ title: '请选择驾龄', icon: 'none', duration: 1500, });
            return;
        }
        if (cityId < 1) {
            Taro.showToast({ title: '请选择城市', icon: 'none', duration: 1500, });
            return;
        }
        if (phoneNumber.length < 11) {
            Taro.showToast({ title: '请输入正确的手机号', icon: 'none', duration: 1500, });
            return;
        }
