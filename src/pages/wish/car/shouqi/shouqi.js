...
export default class Shouqi extends Component {
    ...
    componentDidMount() {
        ...
        window.onresize = function () {
            if (document.body.scrollHeight < h)
                document.getElementsByClassName("Shouqi_btn_submit_wrap")[0].style.display = "none";
            else
                document.getElementsByClassName("Shouqi_btn_submit_wrap")[0].style.display = "block";
        };
    }
        ...
        return (
            <View className='Shouqi'>
                ...
                <View className='Shouqi_content_wrap'>
                    ...
                    <AtInput className='' value={name}  placeholder='请输入真实姓名' maxLength='8'
                        onBlur={this.onInputBlur.bind(this)} onChange={this.setNameValue.bind(this)} />

                    <View className='Shouqi_type_title'>手机</View>
                    <Input className='Shouqi_input' value={phoneNumber} type='number' maxLength='11' placeholder='请输入手机号码'
                        onBlur={this.onInputBlur.bind(this)} onChange={this.setPhoneNumber.bind(this)} />
    ...
    submitBtn(e) {
        ...
        if (name.length < 1) {
            Taro.showToast({ title: '请输入您的姓名', icon: 'none', duration: 3000 });
            return;
        }
        if (phoneNumber.length < 11) {
            Taro.showToast({ title: '请输入正确的手机号', icon: 'none', duration: 3000 });
            return;
        }
        if (cityId <= 0) {
            Taro.showToast({ title: '请选择您所在的城市', icon: 'none', duration: 2000 });
            return;
        }
