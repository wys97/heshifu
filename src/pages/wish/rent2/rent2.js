...
export default class Rent extends Component {
  ...
  state = {
    brands: [{ value: 1, text: '比亚迪', checked: false },
    { value: 2, text: '本田', checked: false },
    { value: 3, text: '日产', checked: false },
    { value: 4, text: '雪铁龙', checked: false },
    { value: 5, text: '荣威', checked: false },
    { value: 6, text: '大众', checked: false }],
    otherBrandChecked: false,
    otherBrand: '',

    dynamics: [{ value: 0, text: '不限' },
    { value: 1, text: '汽油' },
    { value: 2, text: '纯电动' },
    { value: 3, text: '油电混合' },
    { value: 4, text: '油气混合' }],
    ...
    gearboxes: [{ value: 0, text: '不限' },
    { value: 1, text: '自动挡' },
    { value: 2, text: '手动挡' }],
    gearboxIndex: 0,

    displacements: [{ value: 0, text: '不限' },
    { value: 1, text: '1-2L' },
    { value: 2, text: '2-3L' },
    { value: 3, text: '3L以上' }],
    displacementIndex: 0,

    ages: [{ value: 0, text: '不限' },
    { value: 1, text: '新车' },
    { value: 2, text: '6个月以下' },
    { value: 3, text: '6-12个月' },
    { value: 4, text: '1-2年' },
    ...
    features: [{ value: 1, text: '操控好', checked: false },
    { value: 2, text: '油耗低', checked: false },
    { value: 3, text: '故障少', checked: false },
    { value: 4, text: '空间大', checked: false }],

    monies: [{ value: 0, text: '不限', checked: false },
    { value: 1, text: '2000~4000元', checked: false },
    { value: 2, text: '4000~6000元', checked: false },
    { value: 3, text: '6000元以上', checked: false }],
    moneyIndex: 0,

    times: [{ value: 0, text: '不限', checked: false },
    { value: 1, text: '3个月以下', checked: false },
    { value: 2, text: '3~6个月', checked: false },
    { value: 3, text: '6~12个月', checked: false },
    { value: 4, text: '12~24个月', checked: false },
    { value: 5, text: '24个月以上', checked: false }],
    timeIndex: 0,
  ...
  componentDidMount() {
    ...
    window.onresize = function () {
      if (document.body.scrollHeight < h)
        document.getElementsByClassName("rent2-submit")[0].style.display = "none";
      else
        document.getElementsByClassName("rent2-submit")[0].style.display = "block";
    };
  }
  ...
  render() {
    ...
    return (
      <View className='container-rent2'>
          ...
          <View className='rent2-tv-content'>
            ...
            <View className='rent2-other-view'>
              ...
              <View className='rent2_other-input-view'>
                <AtInput className='rent2-tvcv-input' type='text' placeholder='请输入品牌名称' border={false} maxLength={15}
                  onFocus={this.onInputFocus.bind(this)} onBlur={this.onInputBlur.bind(this)}
                  value={otherBrand} onChange={this.onOtherBrandChange.bind(this)} />
        ...
        <View className='rent2-name-view'>
          ...
          <AtInput className='rent2-nv-name-input' type='text' placeholder='请输入您称呼' border={false} maxLength={10}
            onFocus={this.onInputFocus.bind(this)} onBlur={this.onInputBlur.bind(this)}
        ...
        <View className='rent2-name-view bottom-gap'>
          ...
          <AtInput className='rent2-nv-name-input' type='phone' placeholder='请输入联系方式' border={false} maxLength={11}
            onFocus={this.onInputFocus.bind(this)} onBlur={this.onInputBlur.bind(this)}
  ...
  onDynamicChange(e) {
    if (typeof e.detail.value == 'number')
      this.setState({ dynamicIndex: e.detail.value })
    else if (typeof e.detail.value == 'object')
      this.setState({ dynamicIndex: e.detail.value[0] })
  }

  onGearboxChange(e) {
    if (typeof e.detail.value == 'number')
      this.setState({ gearboxIndex: e.detail.value })
    else if (typeof e.detail.value == 'object')
      this.setState({ gearboxIndex: e.detail.value[0] })
  }

  onDisplacementChange(e) {
    if (typeof e.detail.value == 'number')
      this.setState({ displacementIndex: e.detail.value })
    else if (typeof e.detail.value == 'object')
      this.setState({ displacementIndex: e.detail.value[0] })
  }

  onAgeChange(e) {
    if (typeof e.detail.value == 'number')
      this.setState({ ageIndex: e.detail.value })
    else if (typeof e.detail.value == 'object')
      this.setState({ ageIndex: e.detail.value[0] })
  }
  ...
  onMoneyChange(e) {
    if (typeof e.detail.value == 'number')
      this.setState({ moneyIndex: e.detail.value })
    else if (typeof e.detail.value == 'object')
      this.setState({ moneyIndex: e.detail.value[0] })
  }

  onTimeChange(e) {
    if (typeof e.detail.value == 'number')
      this.setState({ timeIndex: e.detail.value })
    else if (typeof e.detail.value == 'object')
      this.setState({ timeIndex: e.detail.value[0] })
  }
  ...
  onSubmit(value) {
    ...
    if (otherBrandChecked)
      brandArray.push(otherBrand);

    if (brandArray.length <= 0) {
      Taro.showToast({ title: '请勾选品牌偏好', icon: 'none', duration: 2000 });
      return;
    }
    ...
    if (featureArray.length <= 0) {
      Taro.showToast({ title: '请勾选车款亮点', icon: 'none', duration: 2000 });
      return;
    }

    if (name.length < 1) {
      Taro.showToast({ title: '请输入您的称呼', icon: 'none', duration: 2000 });
      return;
    }
