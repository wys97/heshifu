...
import { View, Text, Image, Swiper, SwiperItem, Radio, RadioGroup, Label } from '@tarojs/components';
...
export default class Lend extends Component {
  ...
  state = {
    times: [{ value: 1, text: '1~30天' },
    { value: 2, text: '30天~1年' },
    { value: 3, text: '1年以上' }],
    timesIndex: -1,

    amounts: [{ value: 1, text: '1000 ~ 1万' },
    { value: 2, text: '1万 ~ 5万' },
    { value: 3, text: '5万以上' }],
    amountsIndex: -1,

    preferences: [{ value: 1, text: '纯信用借款产品' },
    { value: 2, text: '抵押借款产品（汽车、房产）' }],
    preferencesIndex: -1,

    repayments: [{ value: 1, text: '按日计息，随借随还' },
    { value: 2, text: '按月等额，长期超划算' },
    { value: 3, text: '按月付息，到期还本' },
    { value: 4, text: '一次性还本付息' }],
    repaymentsIndex: -1,

    works: [{ value: 1, text: '出租车司机' },
    { value: 2, text: '网约车司机' }],
    worksIndex: -1,

    earnings: [{ value: 1, text: '5000~8000元' },
    { value: 2, text: '8000~12000元' },
    { value: 3, text: '12000~15000元' },
    { value: 4, text: '15000元以上' }],
    earningsIndex: -1,

    emergencys: [{ value: 1, text: '越快越好，即刻需求' },
    { value: 2, text: '近期有可能需要' },
    { value: 3, text: '暂时不需要' }],
    emergencysIndex: -1,
  ...
  componentDidMount() {
    ...
    window.onresize = function () {
      if (document.body.scrollHeight < h)
        document.getElementsByClassName("money-submit")[0].style.display = "none";
      else
        document.getElementsByClassName("money-submit")[0].style.display = "block";
    };
  }
  ...
  render() {
    ...
    return (
      <View className='container-money  bottom-gap'>
        ...
        <View className='lend-name-view'>
          ...
          <AtInput className='nv-name-input' type='text' placeholder='请输入真实姓名' border={false} maxLength={10} adjustPosition={false}
        ...
        <View className='lend-name-view'>
          ...
          <AtInput className='nv-name-input ' type='phone' placeholder='请输入联系方式' placeholderStyle='color:#F2D3AB' border={false} maxLength={11} adjustPosition={false}
            onBlur={this.onInputBlur.bind(this)} value={phoneNumber} onChange={this.onPhoneNumberChange.bind(this)} />
  ...
  onSubmit(value) {
    ...
    if (timesIndex < 0) {
      Taro.showToast({ title: '请选择周转周期', icon: 'none', duration: 2000 });
      return;
    }
    if (amountsIndex < 0) {
      Taro.showToast({ title: '请选择周转金额', icon: 'none', duration: 2000 });
      return;
    }
    if (preferencesIndex < 0) {
      Taro.showToast({ title: '请选择产品偏好', icon: 'none', duration: 2000 });
      return;
    }
    if (repaymentsIndex < 0) {
      Taro.showToast({ title: '请选择还款方式偏好', icon: 'none', duration: 2000 });
      return;
    }
    if (worksIndex < 0) {
      Taro.showToast({ title: '请选择您的工作', icon: 'none', duration: 2000 });
      return;
    }
    if (earningsIndex < 0) {
      Taro.showToast({ title: '请选择您的月收入范围', icon: 'none', duration: 2000 });
      return;
    }
    if (emergencysIndex < 0) {
      Taro.showToast({ title: '请选择您周转需求的紧急程度', icon: 'none', duration: 2000 });
      return;
    }

    if (name.length < 1) {
      Taro.showToast({ title: '请输入称呼', icon: 'none', duration: 2000 });
      return;
    }
